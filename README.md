...
- `bun run vite:preview`: – превью `production-версии` проекта (рекомендуется запускать после выполнения команды `bun run vite:build`)
- `bun run storybook`: – запуск Storybook компонентов`)
